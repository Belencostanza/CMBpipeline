...
def make_maps(cls, nside):
    #np.random.seed(1234)
    resolution = hp.nside2resol(nside, arcmin = True)
    maps = hp.synfast(cls, nside=nside, new=True, pol=True) # acá esta pol == False
...
def transf_eb_np(qmap, umap, nx, dx):
...
def power_spectrum_flat(cl, nx, dx):
...
def calculate_res(z, nx):

    res_x = (z[:, 0].max() - z[:, 0].min()) / nx
    res_y = (z[:, 1].max() - z[:, 1].min()) / nx
    res = (res_x + res_y) / 2
...
def compute_bins_hybrid(lmin, lmax, Nbins, l_transition=50):
...
def bineado(ell, cl, bins): 
    ...
    #print(f"(bineado eficiente) Execution Time: {end_time - start_time} seconds")
